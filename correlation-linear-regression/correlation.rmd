...
## sen_ins  <- floor(rnorm(n, 250 + 50 * (type=="II"), 25))
## fat_acid <- rnorm(n, 15 + sen_ins/100)
## chl      <- rnorm(n, 120 - exp(scale(sen_ins)), .5)
## bmi      <- rnorm(n, 25, 3)
## enos     <- sample(c("aa", "Aa", "AA"), n, replace = T, prob = hwfq(0.9))
## enos_rec <- enos == "AA"
## bp_means <- 110 - 5*enos_rec + 10*scale(bmi) - 3*enos_rec*scale(bmi) + 5*scale(chl)
## bp       <- floor(rnorm(n, bp_means, 3))
## lod_hatk <- (type=="II") - enos_rec - 0.5*enos_rec*scale(bmi) + scale(bmi) + scale(chl) + rnorm(n)
## hrt_attack <- ifelse(plogis(lod_hatk) < .65, "Sim", "Não")

## db <- data.frame(type, sen_ins, fat_acid, chl, bmi, enos, bp, hrt_attack)
...
Quando deseja-se avaliar como a variação de uma medida afeta outra medida
quantitativa, avalia-se a correlação linear das medidas calculando o
coeficiente de correlação de Pearson ($r$).

$$
    r = \dfrac{cov_{xy}}{s_x * s_y} =
xs        \dfrac{\sum (x-\bar{x})(y-\bar{y})}
...
investigar a concentração de colesterol no sangue. Na mesma amostra, os
pesquisadores obtiveram os dados da concentração de colesterol em jejum.
